...
#[Group('franken-spanner-smoke')]
final class FrankenSpannerEmulatorSmokeTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        if (defined(Channel::class . '::CODEX_TEST_STUB')) {
            self::fail('The FrankenGrpc test stub is loaded; run this suite through tools/franken-smoke.sh.');
        }

        $host = getenv('SPANNER_EMULATOR_HOST');
        if (!is_string($host) || $host === '') {
            if (getenv('SPANNER_SMOKE_SKIP_MISSING_EMULATOR') === '1') {
                self::markTestSkipped('SPANNER_EMULATOR_HOST is required for the Franken Spanner smoke suite.');
            }

            self::fail('SPANNER_EMULATOR_HOST is required for the Franken Spanner smoke suite.');
        }

        $suffix = strtolower(bin2hex(random_bytes(4)));
        $this->instanceId = 'franken-gax-smoke-' . $suffix;
        $this->databaseId = 'frankensmokedb' . $suffix;
